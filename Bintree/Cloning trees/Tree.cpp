...
bool Tree::_identical(Node *nd1, Node *nd2) {
    if(nd1 == nullptr && nd2 == nullptr){
        ...
        if(nd1->key == nd2->key){
            //o "&&" serve para retornar true e ainda continuar executando a linha de código até achar um false. Caso não encontre, retornará true.
...
Tree *Tree::clone() {
    Tree *copia = new Tree;
    copia->_root = _clone(_root);
    return copia;
}

Node *Tree::_clone(Node *node) {
    //igual a criar uma copia em nós praticamente.
    if(node == nullptr){
        return nullptr;
    }else{
        Node *copia = new Node(node->key);
        Node *c_e = _clone(node->left);
        Node *c_d = _clone(node->right);
        copia->left = c_e;
        copia->right = c_d;
        return copia;
    }
